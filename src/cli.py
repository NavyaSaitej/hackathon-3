...
def ingest(
    ...
    with Progress(
        ...
        progress.add_task(description=f"Ingesting [bold cyan]{file_path.name}[/bold cyan]...", total=None)
...
def list():
    ...
    for note in notes:
        summary_snippet = (note.summary[:60] + "...") if note.summary and len(note.summary) > 60 else (note.summary or "")
        table.add_row(
            str(note.id),
            note.filename,
...
@app.command()
def view(
    note_id: int = typer.Argument(..., help="The ID of the archive to view.")
):
    """
...
@app.command()
def delete(
    note_id: int = typer.Argument(..., help="The ID of the archive to delete.")
):
    """
