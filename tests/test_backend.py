...
from backend.exceptions import ConfigurationError, ParserFailureError
...
@patch('backend.parsers.Path.exists', return_value=False)
...
@patch('backend.parsers.Path.exists', return_value=True)
@patch('backend.parsers.Path.read_text', return_value="Hello World")
